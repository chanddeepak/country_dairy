...
/**
 * Everything the page needs, worked out from the API's shape.
 *
 * Pure, and outside the component, so the very first render can use it. It
 * used to run inside an effect, which meant the server sent "Loading product
 * details" and the real page only existed after the browser had fetched,
 * parsed and re-rendered — a crawler without JavaScript never saw a product
 * at all, and the hero image could not start downloading until all of that
 * had happened.
 */
function normaliseProduct(prod: any, variantIdFromQuery?: string | null) {
  ...
  return { normalizedProd, defaultVar, activeImage: resolveStorefrontImageUrl(initialImg) };
}

export default function ProductDetailClient({
  initialProduct,
}: {
  /** Fetched on the server so the first response carries the product. */
  ...
  /*
   * Seeded, not empty. These three used to start blank and be filled by an
   * effect, which is what made the server send a shell — with the product in
   * hand at render time the markup is complete in the first response.
   */
  const seed = useMemo(
    () => (initialProduct ? normaliseProduct(initialProduct, variantIdFromQuery) : null),
    [initialProduct, variantIdFromQuery],
  );

  const [product, setProduct] = useState<Product | null>((seed?.normalizedProd as Product) ?? null);
  ...
  /*
   * The server already fetched this product, so the first pass only needs the
   * related-products call. Re-fetching here would throw away a complete render
   * and put the loading state back on screen for no new information.
   */
  const alreadyHaveIt = useRef(Boolean(initialProduct));

  useEffect(() => {
    if (alreadyHaveIt.current) {
      alreadyHaveIt.current = false;
      void loadRelated(initialProduct);
      return;
    }
    if (slug) {
      fetchProduct();
  ...
  const setupProductData = (prod: any) => {
    if (!prod) return;
    const { normalizedProd, defaultVar, activeImage } = normaliseProduct(prod, variantIdFromQuery);

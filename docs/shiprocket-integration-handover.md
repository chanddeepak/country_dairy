...
We understand from your documentation that the **API Key, Password, Shared
Secret and Storefront Access Token are generated by Shiprocket** and appear
pre-populated in the checkout dashboard, rather than being chosen by the
merchant. On that basis:

| Direction | Which credentials | What we need |
| --- | --- | --- |
| **You → us** (catalogue pulls) | the pair your sync will present | tell us the values, and we will configure our endpoints to accept exactly those |
| **Us → you** (checkout token, order lookup) | the pair your API verifies | the values |

If both directions share a single pair, please say so. Our implementation reads
one key and one secret from configuration, so either arrangement works — we
only need to know which values to expect and which to send.
